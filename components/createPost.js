...
import { postBaseUrl } from "./requests";
...
if (response) {
   ...
   submit(feedback) {
    ...
    const { message, title, image, username, doc,docUri,userType } = this.state;
  if (message != null) {
  ...
  // console.log('filename', splitTest(image[0].uri))
    ...
    // upload.append('teacher_id',teacher_id) 
    upload.append('doc',docUri) 

   
      this.props.onSubmit(upload,feedback);
      let from = this.props.navigation
  ...
  async componentDidMount() {
    try {
      ...
      const uid = this.props.navigation.getParam('userId')
        let response = await fetch(`${postBaseUrl}/user/${uid}`);
        var json = await response.json()
        
          const img = await json
          console.log('pp',img[0].avatar)
         this.setState({avatar:img[0].avatar})
